...
UHTPatch({
	...
	apply:function()
	{
		BetLevelV2.prototype.OnBetChanged = function()
		{
			...
			var betOnLvlIndex1 = bets[XT.GetInt(Vars.NextBetIndex)] * this.betLevelSettings.betLevelScale[1];
			this.willExcedeMaxTotalBet = (betOnLvlIndex1 > maxTotalBet);

			XT.SetBool(Vars.MaxCoinValueReached, XT.GetInt(Vars.CoinValueIndex) + 1 == coinValues.length);
			XT.SetBool(Vars.MaxBetReached, XT.GetInt(Vars.NextBetIndex) + 1 == bets.length);
			XT.SetBool(Vars.MaxBetAndCoinValueReached, XT.GetBool(Vars.MaxBetReached) && XT.GetBool(Vars.MaxCoinValueReached));

			if (this.betLevelSettings.betLevelIndex == 0)
			{
				if (this.willExcedeMaxTotalBet)
				{
					if (XT.GetBool("Jurisdiction_HideAnteBet"))
						this.featureIsAvailable.hide.Start();
				}
				else
				{
					if (XT.GetBool("Jurisdiction_HideAnteBet"))
						this.featureIsAvailable.show.Start();
				}
			}
			else
			{
				if (XT.GetInt(Vars.CoinValueIndex) + 1 < coinValues.length)
				{
					var betMultipleOnCoinIncrease = coinValues[XT.GetInt(Vars.CoinValueIndex) + 1] / coinValues[XT.GetInt(Vars.CoinValueIndex)];
					XT.SetBool(Vars.MaxCoinValueReached, (betOnLvlIndex1 * betMultipleOnCoinIncrease) - 1e-3 > maxTotalBet);
				}

				if (XT.GetInt(Vars.NextBetIndex) + 1 < bets.length)
				{
					var betMultipleOnBetIndexIncrease = bets[XT.GetInt(Vars.NextBetIndex) + 1] / bets[XT.GetInt(Vars.NextBetIndex)];
					XT.SetBool(Vars.MaxBetReached, (betOnLvlIndex1 * betMultipleOnBetIndexIncrease) - 1e-3 > maxTotalBet);
				}

				if (this.willExcedeMaxTotalBet)
				{
					XT.TriggerEvent(Vars.Evt_DataToCode_SmartDecreaseBet);
				}
			}

			this.UpdateOptionsCost();
...
UHTPatch({
	name: "PatchBetLevelManagerMaxBet",
	ready:function()
	{
	...
	apply:function()
	{
		BetLevelManager.prototype.OnBetChanged = function()
		{
			...
			if (this.betLevelSettings.betLevelIndex == 0)
			{
				if (betOnLvlIndex1 > maxTotalBet)
				{
					this.forceDisableBetLevelChanges = true;
					this.DisableBetLevelChanges();
					if (XT.GetBool("Jurisdiction_HideAnteBet"))
				...
				{
					if (!this.duringFRB)
						this.forceDisableBetLevelChanges = false;

					if (XT.GetBool("Jurisdiction_HideAnteBet"))
						Globals.RestoreCachedLayersRecursively(this.gameObject);

					this.EnableBetLevelChanges();
				}
			}
			else
			{
			...
			this.betLevelDisplayer.UpdateBetLevels(this.betLevelSettings.betLevelScale, this.betLevelSettings.betLevelIndex);
		};
	},
	retry:function()
	{
...
UHTPatch({
	name: "PatchTurboTumble",
	ready:function()
	{
		return (window["StageResult"] != undefined);
	},
	apply:function()
	{
		if (window["TumblingManager"] == undefined)
			return;
		...
		StageResult.prototype.CanSpin = function()
		{
			...
			if (XT.GetBool(Vars.FeaturePurchaseWindowIsOpen))
				return false;
			...
			if(this.isRespin)
			{
				if (this.IsAutoRespin() && !(XT.GetBool(Vars.IsDifferentSpinType) && XT.GetBool(Vars.ContinuousSpin)))
					return false;
				else
					return true;
			}
			...
			if (JackpotsManager.I)
				if (JackpotsManager.I.hasActiveJp && !JackpotsManager.I.hasOpenJp)
				{
			...
			if (XT.GetBool(Vars.SpinNeedsConfirmation))
			{
				if (this.autoplayRequested)
					this.mustStartAutoplayAfterConfirmSpinStart = true;
		...
		TumblingManager.prototype.Update = function()
		{
			if (!(this.isTumbling && this.resultReceived))
				return;

			var i;
			var generalState = this.tumblers[this.oldTumblingData.tumblingSymbols[0].position].tumblingState;

			if (generalState == SymbolTumbler.State.removing || generalState == SymbolTumbler.State.tumbling)
				return;

			for (i = 1; i < this.oldTumblingData.tumblingSymbols.length; i++)
			{
				if (this.tumblers[this.oldTumblingData.tumblingSymbols[i].position].tumblingState != generalState)
					return;
			}

			if(generalState == SymbolTumbler.State.removed)
			{
				var screenSymbols = XT.GetObject(Vars.InitialResultScreenSymbols);
				var reelIdx;
				var holderIdx;
				XT.SetBool(Vars.CanUpdateLinkedLoopersPosition, false);
				var reelDelays = _array.create(this.reelsManager.reels.length);
				var tumbledReels = 0;

				for(var j = 0; j < reelDelays.length; j++)
				{
					if (this.ReplacedOnReel(j) > 0 && !(XT.GetBool(Vars.ContinuousSpin)))
					{
						reelDelays[j] = (tumbledReels == 0) ? -1 : tumbledReels * this.delayBetweenReels;
						tumbledReels++;
					}
					else
					{
				...
				for (i = 0; i < this.tumblers.length; i++)
				{
					reelIdx = i % this.reelsManager.reels.length;
					holderIdx = Math.floor(i / this.reelsManager.reels.length);
					var symbolID = screenSymbols[reelIdx][this.reelsManager.reels[0].symbolHolders.length - holderIdx - 1];

					if (holderIdx <= this.GreatestReplacedIdx(reelIdx))
					{
						if (this.spinInPlaceWillNotFall && this.reelsManager.reels[reelIdx].spinInPlace && !this.posIsReplaced[reelIdx][holderIdx])
							continue;

						this.tumblers[i].TumbleNewSymbol(symbolID, this.CalculateOffset(reelIdx, holderIdx), reelDelays[reelIdx]);
					}
				}
				
				if (this.TumbleStarted != undefined)
					this.TumbleStarted(reelDelays);

				return;
			}

			if (generalState == SymbolTumbler.State.tumbled)
			{
				for (i = 0; i < this.oldTumblingData.tumblingSymbols.length; i++)
				{
					this.tumblers[this.oldTumblingData.tumblingSymbols[i].position].tumblingState = SymbolTumbler.State.idle;
				}

				this.AnounceSpinEnded();
			}
		};
		if (window["TumblingManager_GRM"] != undefined)
			TumblingManager_GRM.prototype.TumbleScreen = function()
			{
				this.rq.StopRoutines();

				if (this.rm.stopOnInitialScreenSymbols)
					this.ss = XT.GetObject(Vars.InitialResultScreenSymbols);
				else
					this.ss = XT.GetObject(Vars.ResultScreenSymbols);

				if (XT.GetBool(Vars.Logic_IsFreeSpin))
					this.rq.AddRoutineQueueItem(new RQIWaitForXTEventWithCallback("MultiplierUpdateIsDone_GRM", "CheckForWinMultiplier_GRM"));

				for (var rIndex = 0; rIndex < this.tumbleScreen.length; rIndex++)
				{
					if(this.ReelHasTumble(rIndex))
					{
						this.symbolHeight = this.reels[rIndex].reelHeight / this.reels[rIndex].GetReelSize();
						var symbol = null;

						//existing screen symbols
						for (var sIndex = this.reels[rIndex].GetReelSize() - 1; sIndex >= 0; sIndex--)
						{
							symbol = this.reels[rIndex].symbolHolders[sIndex].currentSymbol;
							if(symbol == null)
							{
								var saIndex = sIndex;
								while (symbol == null && saIndex > 0)
								{
									saIndex--;
									symbol = this.reels[rIndex].symbolHolders[saIndex].currentSymbol;
								}

								if(symbol != null)
								{
									this.reels[rIndex].symbolHolders[sIndex].currentSymbol = symbol;
									symbol.transform.SetParent(this.reels[rIndex].symbolHolders[sIndex].transform, true);
									this.reels[rIndex].symbolHolders[saIndex].currentSymbol = null;

									this.st = symbol.GetComponent(SymbolTumbler_GRM);
									this.st.SetTumblePoints(this.st.transform.localPosition().y, this.tumbleScreen[rIndex][saIndex] * this.symbolHeight);

									this.rq.AddRoutineQueueItem(new RQITriggerCallback(this.TumbleSymbol, this.st, this));
									if(this.delayBetweenTumbles > 0 && !(XT.GetBool(Vars.ContinuousSpin)))
										this.rq.AddRoutineQueueItem(new RQIWaitForSeconds(this.delayBetweenTumbles));
								}
							}
						}

						//extra top symbols
						for (var sIndex = this.tumbleScreen[rIndex][0] - 1; sIndex >= 0; sIndex--)
						{
							var sh = this.reels[rIndex].symbolHolders[sIndex];
							var newSymbolId = this.ss[rIndex][this.ss[rIndex].length - sIndex - 1];
							
							symbol = this.rm.symbolsManager.GetSymbol(newSymbolId, this.newSymbolsParent);
							symbol.SpinStop();
							sh.currentSymbol = symbol;
							symbol.transform.SetParent(sh.transform, true);
							this.st = symbol.GetComponent(SymbolTumbler_GRM);
							this.st.ssCAT.SetSize(this.reels[rIndex].GetReelSize() - 1);
							symbol.transform.localPosition(new UHTMath.Vector3(0, 5000, 0));
							this.posOffset = this.symbolHeight * (this.tumbleScreen[rIndex][0] - sIndex) - sh.transform.localPosition().y;
							this.st.SetTumblePoints(this.posOffset, this.tumbleScreen[rIndex][0] * this.symbolHeight);
				...
				this.rq.AddRoutineQueueItem(new RQIWaitForSeconds(this.delayAfterFinalTumble));
				this.rq.AddRoutineQueueItem(new RQITriggerCallback(this.TriggerTumbleSpinEnded, null, this));
				this.rq.StartRoutines();
			};
		if (window["TopReel_GRM"] != undefined)
			TopReel_GRM.prototype.TumbleScreen = function()
			{
				this.rq.StopRoutines();
						
				if (this.tumbleScreen[0] > 0)
				{
					//existing screen symbols
					for (var sIndex = this.srm.reels[0].symbolHolders.length - 1; sIndex >= 0; sIndex--)
					{
						var srsh = this.srm.reels[0].symbolHolders[sIndex];

						if(srsh.currentSymbol == null)
						{
							var saIndex = sIndex;
							while(srsh.currentSymbol == null && saIndex > 0)
							{
								saIndex--;
								srsh.currentSymbol = this.srm.reels[0].symbolHolders[saIndex].currentSymbol;
							}

							if(srsh.currentSymbol != null)
							{
								this.srm.reels[0].symbolHolders[saIndex].currentSymbol = null;
								srsh.currentSymbol.transform.SetParent(srsh.transform, true);
								srsh.currentSymbol.transform.localScale(1, 1, 1);
								this.st = srsh.currentSymbol.GetComponent(SymbolTumbler_GRM);
								this.st.SetTumblePoints(this.st.transform.localPosition().y, this.tumbleScreen[saIndex] * this.symbolWidth);
								this.rq.AddRoutineQueueItem(new RQITriggerCallback(this.TumbleSymbol, this.st, this));
								if(this.tm.delayBetweenTumbles > 0 && !(XT.GetBool(Vars.ContinuousSpin)))
									this.rq.AddRoutineQueueItem(new RQIWaitForSeconds(this.tm.delayBetweenTumbles));
							}
						}
					}

					//extra top symbols
					for (var sIndex = this.tumbleScreen[0] - 1; sIndex >= 0; sIndex--)
					{
						var srsh = this.srm.reels[0].symbolHolders[sIndex];

						var newSymbolId = this.screenSymbols[0][this.screenSymbols[0].length - sIndex - 1];
						this.posOffset = this.symbolWidth * this.tumbleScreen[0];

						var symbol = this.srm.symbolsManager.GetSymbol(newSymbolId, this.tm.newSymbolsParent);
						symbol.SpinStop();
						symbol.transform.SetParent(srsh.transform, true);
						symbol.transform.localScale(1, 1, 1);
						symbol.transform.localPosition(new UHTMath.Vector3(0, 5000, 0));
						symbol.transform.localRotation(UHTMath.Quaternion.euler(UHTMath.Vector3.zero));
						srsh.currentSymbol = symbol;

						this.st = symbol.GetComponent(SymbolTumbler_GRM);
						this.st.SetTumblePoints(this.posOffset, this.tumbleScreen[0] * this.symbolWidth);
				...
				this.rq.AddRoutineQueueItem(new RQIWaitForSeconds(this.tm.delayAfterFinalTumble));
				this.rq.AddRoutineQueueItem(new RQITriggerCallback(this.TriggerTumbleSpinEnded, null, this));
				this.rq.StartRoutines();
			};
		if (window["TumblingManager_CC"] != undefined)
			TumblingManager_CC.prototype.TumbleScreen = function()
			{
				this.rq.StopRoutines();
				...
				this.rq.AddRoutineQueueItem(new RQIWaitForXTEventWithCallback("MultiplierUpdateIsDone_CC", "CheckForWinMultiplier_CC"));
		...
		if (window["TumblingManager_BUKIM"] != undefined)
			TumblingManager_BUKIM.prototype.TumbleScreen = function()
			{
				this.rq.StopRoutines();
				var ss = XT.GetObject(Vars.ResultScreenSymbols);
				...
				for (var rIndex = 0; rIndex < this.tumbleScreen.length; rIndex++)
				{
					if(this.ReelHasTumble(rIndex))
					{
						...
						{
							var sh = this.reels[rIndex].symbolHolders[sIndex];
							var newSymbolId = ss[rIndex][ss[rIndex].length - sIndex - 1];
					...
					if (!prepared) //You are not prepared!
					{
						...
						if (window["JurisdictionGameTitle"] != undefined)
						{
							...
							if (jurisdictionGameTitle != null)
							{
								...
								var portraitGT = window["globalRuntime"].sceneRoots[1].transform.Find("UI Root/XTRoot/Root/GUI_mobile/PragmaticPlay/Port/Arrangeable/GT");
								if (portraitGT != null)
								{
...
UHTPatch({
	...
	apply:function()
	{
		if (IsRequired("SOCE"))
		{
			...
			{
				SOCE_replacements.splice(0,0,{src:"pay", dst:"play"});
